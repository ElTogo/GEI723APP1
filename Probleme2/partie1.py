...
El = -74*mV
taue = 5*ms
F = 15*Hz # Changer la fréquence pour 10 et 6 Hz pour la partie I.2.
gmax = .01
...
on_pre_aI = '''ge += w; Apre += dApre_aI; w = clip(w + Apost, 0, gmax)'''
on_post_aI = '''Apost += dApost_aI; w = clip(w + Apre, 0, gmax)'''
...
on_pre_aII = '''ge += w; A_slow += dA_slow; w = clip(w + A_fast, 0, gmax)'''
on_post_aII = '''A_fast += dA_fast; w = clip(w - A_slow, 0, gmax)'''
...
on_pre_bI = '''ge += w; A += dA_bI; w = clip(w + A, 0, gmax)'''
on_post_bI = '''A += dA_bI; w = clip(w + A, 0, gmax)'''
...
on_pre_bII = '''ge += w; Apre += dApre_LTD; w = clip(w + Apost, 0, gmax)'''
on_post_bII = '''Apost += dApost_LTP; w = clip(w + Apre, 0, gmax)'''


# Changer eqs_x, on_pre_x et on_post_x pour changer de STDP
S = Synapses(poisson_input, neurons, eqs_aI, on_pre=on_pre_aI, on_post=on_post_aI)
